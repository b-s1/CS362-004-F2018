...
int adventurerFunction(int currentPlayer, struct gameState *state )
{
    ...
    while(drawntreasure<2){
        //shuffle discard and place in deck if empty
        if (state->deckCount[currentPlayer] < 1)
            shuffle(currentPlayer, state);
        drawCard(currentPlayer, state);
        //recent card is top hand
        cardDrawn = state->hand[currentPlayer][state->handCount[currentPlayer]-1];
        if (cardDrawn == gold || cardDrawn == silver || cardDrawn == copper)
            //bug: 2 treasure instead of 1
...
int council_roomFunction(int currentPlayer, struct gameState *state, int handPos)
{
    ...
    //place used card in used card pile
    discardCard(handPos, currentPlayer, state, 0);
    return 0;
}
...
int minionFunction(int currentPlayer, struct gameState *state, int choice1, int choice2, int handPos)
{
    ...
    //bug: 2 actions instead of 1
    state->numActions++;
    state->numActions++;
    
    //discard card from hand
    discardCard(handPos, currentPlayer, state, 0);
    //bug: only 1 coin instead of 2
    if (choice1)		
    {
    ...
    else if (choice1)
    {
        ...
        //other players clear hand and redraw if current hand over 4 cards
        for (i = 0; i < state->numPlayers; i++)
        {
            if (i != currentPlayer)
            {
